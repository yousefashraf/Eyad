...
await initDb();
const db = getDb();
const admin = db.prepare("SELECT id FROM users WHERE lower(email) = 'eyad.bassem98@hotmail.com'").get();
if (!admin) {
...
db.prepare(
  "UPDATE users SET password_hash = ?, role = 'admin' WHERE lower(email) = 'eyad.bassem98@hotmail.com'",
).run(passwordHash);
